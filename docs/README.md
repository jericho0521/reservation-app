...
This directory documents the Docker-first, single-business reservation
platform. The current product surface includes the standalone `/v1` API,
background worker, owner console, public booking application, database bundle,
and the public SDK, contract, React, and UI packages.
...
| Review historical backend extraction decisions | [Archived backend extraction material](package-refactor/README.md) |
...
| Supabase reference | [supabase](supabase) | Legacy and environment-specific Supabase notes; current installation starts with Docker documentation. |
| Historical plans | [superpowers](superpowers) and [package-refactor](package-refactor) | Archived design, planning, and extraction records; not current operator instructions. |
...
- Keep recordings, screenshots, logs, and acceptance evidence under ignored
  `tmp/`; do not commit them as documentation assets.
