...
export default async function ConversationsPage({ searchParams }: { searchParams: Promise<{ channel?: string }> }) {
  try {
    ...
    return <div className="page-stack"><header className="page-header split-header"><div><span className="eyebrow">Unified inbox</span><h1>Every customer conversation</h1><p>Web chat, WhatsApp, and simulation share one timeline and one staff takeover state.</p></div><InboxRefresh lastUpdated={result.conversations[0]?.updated_at} /></header><ConversationList conversations={result.conversations} channel={channel} /></div>;
