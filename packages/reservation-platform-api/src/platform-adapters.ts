...
export function toPlatformResource(row: unknown): ResourceResponse {
  ...
  return {
    ...
    kind: resourceKindFrom(record.kind ?? record.resource_kind),
    is_active: record.status === "inactive" ? false : booleanValue(record.is_active, true),
  ...
  return {
    ...
    slots: slots.map((slot): AvailabilitySlot => {
      ...
      return {
        ...
        ...(stringValue(item.staff_id) ? { staff_id: stringValue(item.staff_id) } : {}),
