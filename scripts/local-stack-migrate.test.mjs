...
test("migration planner requires exactly 000001 through 000033 in order", async () => {
  ...
  assert.deepEqual(
    ...
    Array.from({ length: 33 }, (_, index) => String(index + 1).padStart(6, "0")),
  );
  assert.throws(
    () => planCoreMigrations({ ...index, coreMigrations: index.coreMigrations.slice(0, 21) }, []),
    /exactly 33 core migrations/u,
  );
});
