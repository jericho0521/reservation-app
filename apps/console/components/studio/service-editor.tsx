...
export function ServiceEditor({ service, onboarding = false }: { service?: ServiceResponse; onboarding?: boolean }) {
    ...
    <article className={`catalog-editor${archived ? " archived" : ""}`}>
      ...
      <form action={action} className="studio-form" onInput={() => setDirty(true)}>
        ...
        {onboarding ? <>
          ...
          <input name="resource_kind" type="hidden" value="custom" />
          <input name="resource_strategy" type="hidden" value="assigned_resource" />
          <small className="field-hint">Each appointment is assigned to a practitioner in the next step.</small>
        </> : <>
