...
import { filterAppointments } from "../../lib/appointment-view";
...
export default async function ReservationsPage({ searchParams }: { searchParams: Promise<ReservationSearchParams> }) {
  try {
    ...
    return <div className="page-stack"><header className="page-header split-header"><div><span className="eyebrow">Appointment command center</span><h1>Operate the working day</h1><p>Filter the authorized daily schedule, resolve pending work, and move each appointment through an explicit lifecycle.</p><span className="status-pill">{appointments.length} shown</span></div><LiveStatus lastUpdated={appointments[0]?.updated_at ?? overview.generated_at} /></header><ReservationFilters services={services.services} practitioners={practitioners} location={activeLocation} values={values} /><AppointmentCalendar appointments={appointments} date={date} timezone={overview.timezone} practitioners={practitioners} services={services.services} /></div>;
...
function practitionerOptions(reservations: Array<{ staff_id?: string; metadata?: Record<string, unknown> }>, resources: Array<{ resource_id: string; label: string }>) {
  const labels = new Map(resources.map((resource) => [resource.resource_id, resource.label]));
  for (const reservation of reservations) {
    if (!reservation.staff_id) continue;
    const named = reservation.metadata?.staff_name;
    if (!labels.has(reservation.staff_id)) labels.set(reservation.staff_id, typeof named === "string" ? named : `Practitioner ${reservation.staff_id.slice(0, 8)}`);
  }
