...
export function ConversationThread({ conversation, messages }: { conversation: ConversationResponse; messages: ConversationMessageResponse[] }) {
  return <section className="conversation-thread-panel">
    <header><div><span className="eyebrow">{conversationChannelLabel(conversation.channel)}</span><h1>{conversation.participant?.display_name ?? conversation.participant?.contact_hint ?? "Customer conversation"}</h1></div>{conversation.reservation_id ? <a className="secondary-action" href={`/reservations/${encodeURIComponent(conversation.reservation_id)}`}>View reservation</a> : null}</header>
