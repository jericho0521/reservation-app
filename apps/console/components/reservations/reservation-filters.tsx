...
export function ReservationFilters({ services, practitioners, location, values }: { services: ServiceResponse[]; practitioners: Array<{ id: string; label: string }>; location: string; values: ReservationFilterValues }) {
  return <form className="reservation-filters appointment-filters" method="get">
    ...
    {practitioners.length > 0 ? <label>Practitioner<select name="practitioner" defaultValue={values.practitioner ?? ""}><option value="">All authorized</option>{practitioners.map((practitioner) => <option key={practitioner.id} value={practitioner.id}>{practitioner.label}</option>)}</select></label> : null}
    ...
    <label>Channel<select name="channel" defaultValue={values.channel ?? ""}><option value="">All</option><option value="web_booking">Web booking</option><option value="web_chat">Web chat</option><option value="whatsapp">WhatsApp</option><option value="staff">Staff</option><option value="simulation">Simulation</option></select></label>
