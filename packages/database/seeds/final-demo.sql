...
insert into public.services (id, venue_id, name, description, total_seats, resource_kind, selection_mode, reservation_policy, metadata) values
  ('00000000-0000-4000-8000-000000000201', '00000000-0000-4000-8000-000000000101', 'Apex GT Racing Session', 'One-hour simulator session', 3, 'station', 'assigned_resource', '{"kind":"assigned_resource","selection_mode":"assigned_resource","require_resource_labels":true,"allow_partial_capacity":false}', '{"duration_minutes":60}'),
  ('00000000-0000-4000-8000-000000000202', '00000000-0000-4000-8000-000000000102', 'Harbour Meeting Room', 'Two-hour room reservation', 2, 'room', 'assigned_resource', '{"kind":"assigned_resource","selection_mode":"assigned_resource","require_resource_labels":true,"allow_partial_capacity":false}', '{"duration_minutes":120}'),
  ('00000000-0000-4000-8000-000000000203', '00000000-0000-4000-8000-000000000103', 'Luma Consultation', 'Thirty-minute consultation', 2, 'station', 'assigned_resource', '{"kind":"assigned_resource","selection_mode":"assigned_resource","require_resource_labels":true,"allow_partial_capacity":false}', '{"duration_minutes":30}');
insert into public.reservable_resources (id, service_id, label, resource_kind, capacity, sort_order) values
  ...
  ('00000000-0000-4000-8000-000000000306', '00000000-0000-4000-8000-000000000203', 'Specialist Maya', 'station', 1, 1),
  ('00000000-0000-4000-8000-000000000307', '00000000-0000-4000-8000-000000000203', 'Specialist Noah', 'station', 1, 2);

insert into public.bookings (id, service_id, user_name, user_email, user_phone, booking_date, start_time, end_time, seats_booked, seat_labels, status, interface_type, created_at) values
  ('00000000-0000-4000-8000-000000000401', '00000000-0000-4000-8000-000000000201', 'Aina Driver', 'aina@example.test', '+601100000001', current_date, '10:00', '11:00', 1, '{"Simulator A"}', 'confirmed', 'form', now() - interval '2 days'),
  ('00000000-0000-4000-8000-000000000402', '00000000-0000-4000-8000-000000000201', 'Ben Racer', 'ben@example.test', '+601100000002', current_date, '14:00', '15:00', 1, '{"Simulator B"}', 'confirmed', 'chat', now() - interval '1 day'),
  ('00000000-0000-4000-8000-000000000403', '00000000-0000-4000-8000-000000000202', 'Cora Guest', 'cora@example.test', '+601100000003', current_date + 1, '09:00', '11:00', 1, '{"Harbour One"}', 'confirmed', 'form', now()),
  ('00000000-0000-4000-8000-000000000404', '00000000-0000-4000-8000-000000000203', 'Dev Client', 'dev@example.test', '+601100000004', current_date - 1, '15:00', '15:30', 1, '{"Specialist Maya"}', 'cancelled', 'chat', now() - interval '3 days');
