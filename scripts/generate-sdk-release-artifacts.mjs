...
function renderCompatibilityMatrix() {
  const rows = [
    ...
    ["Registry install status", `Disposable strict proof serves exact \`${sdkPackage.name}@${sdkPackage.version}\` and \`${contractPackage.name}@${contractPackage.version}\` artifacts from a temporary local npm-compatible registry, installs them without overrides into an external temp consumer, and typechecks SDK values plus contract type imports. Private registry and public npm publication are not required for the bundled self-hosted release path.`],
