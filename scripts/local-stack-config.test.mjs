...
test("local stack config is generated once and remains stable", async () => {
  ...
  assert.match(first.consoleEnv, /^RESERVATION_SESSION_COOKIE_SECURE=false$/mu);
...
test("existing local stack config is upgraded with local authentication settings", async () => {
  ...
  await writeFile(
    join(directory, "console.env"),
    initial.consoleEnv.replace(/^RESERVATION_SESSION_COOKIE_SECURE=.*\n/mu, ""),
    { mode: 0o600 },
  );
  ...
  assert.match(upgraded.consoleEnv, /^RESERVATION_SESSION_COOKIE_SECURE=false$/mu);
});
...
test("Windows checkouts preserve checksummed SQL and Linux container entrypoints", async () => {
  const [attributes, localDockerfile, apiDockerfile, webDockerfile, toolsDockerfile] = await Promise.all([
    ...
    readFile(new URL("../Dockerfile.production-tools", import.meta.url), "utf8"),
  ]);
  ...
  for (const dockerfile of [localDockerfile, toolsDockerfile]) {
    assert.equal(dockerfile.includes("find packages/database/migrations/supabase -type f -name '*.sql'"), true);
  }
});
