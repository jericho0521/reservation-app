...
test("migration planner requires exactly 000001 through 000037 in order", async () => {
  ...
  assert.deepEqual(
    ...
    Array.from({ length: 37 }, (_, index) => String(index + 1).padStart(6, "0")),
  );
  assert.throws(
    () => planCoreMigrations({ ...index, coreMigrations: index.coreMigrations.slice(0, 21) }, []),
    /exactly 37 core migrations/u,
  );
});
