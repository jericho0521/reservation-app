  ...
  try {
    ...
    return (
      <div className="page-stack">
        <header className="page-header split-header"><div><span className="eyebrow">Operations command center</span><h1>{workspace?.profile.name ?? "Daily operations"}</h1><p>{overview ? `${longDate(overview.local_date)} · ${overview.timezone}` : "Workspace details are available while operational summaries recover."}</p></div>{overview ? <span className="live-refresh">Updated {new Date(overview.generated_at).toLocaleTimeString("en-MY", { hour: "2-digit", minute: "2-digit" })}</span> : null}</header>
