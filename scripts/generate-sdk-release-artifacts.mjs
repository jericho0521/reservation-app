...
function renderReleaseNotes() {
  return [
    ...
    "- Standalone backend health proof is wired through `backend-platform:live-proof:strict`; it passed once against a local `apps/api` Node process outside the current Next.js frontend runtime and validated the `/v1/health` response contract. This is health-only evidence, not DB-backed API parity.",
    ...
    "",
    "- Strict current frontend consumer install/typecheck/build proof has passed once with `CURRENT_FRONTEND_CONSUMER_PROOF_ROOT` pointing at a prepared external frontend workspace and `CURRENT_FRONTEND_CONSUMER_PROOF_ALLOW_INSTALL=1`, using staged SDK and contract package artifacts. Strict disposable registry install/type-import proof has also passed once. These do not prove public/private registry publication, live backend behavior, or frontend runtime smoke.",
    ...
    "- Final standalone frontend/backend extraction and backend release tagging are not complete. The frontend consumer readiness and prepared-artifact install/build proof, extraction manifest, dry-run extraction plan, extracted-workspace readiness model, extracted-install proof harness, SQL ownership inventory, migration bundle manifest, migration index, standalone JWT/JWKS verifier, bounded JWKS cache tests, passed disposable database strict proof, and passed disposable registry strict proof are still short of the final product split; they do not create or populate the future frontend or backend repositories, configure a live auth provider, prove provider operational key rotation, deploy a backend, publish packages, prove public/private registry installation, or prove live parity.",
