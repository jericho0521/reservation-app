...
Next.js for public health/readiness hygiene, metadata, injected catalog
repository reads, injected availability reads, injected read-only reservation
reads, injected idempotent reservation creation, injected idempotent
reservation lifecycle mutations, injected resource-maintenance list plus
idempotent resource-maintenance create/end routes, disabled chat routes, and
backend-only Supabase runtime dependency construction from explicit/env config,
plus optional backend-only service-token auth, provider-neutral JWT/JWKS bearer
verification, bounded JWKS
...
repository extraction. The health endpoints prove only that a host process can
answer a cheap public request when deployed; they do not prove that deployment
has happened or that database/Supabase/RLS parity is live.
...
the expected live env contract on every run through an exported parser with
focused unit tests for trim/normalization, required config, malformed URL/date
and quantity handling, strict-mode failure behavior, and mutation opt-in
readiness. It exits `0` with `SKIPPED` when the required live backend fixture
env is absent or incomplete. If the full env is present, it performs SDK/direct
HTTP parity checks against the same live `/v1` backend for metadata, configured
service, configured resource, availability, and reservation list/summary
responses. Mutation checks run only when `sdk:live-parity:strict` is running
with `RESERVATION_PLATFORM_LIVE_ALLOW_MUTATIONS=1`.
...
This proof does not seed or clean up the backend by itself. Parser unit tests
prove only local harness decision behavior; they do not prove a real live
backend. The strict command must still pass against a disposable seeded backend
slot before live backend parity can be claimed. It also does not prove live
enabled-chat backend/provider parity.
