...
function renderReleaseNotes() {
  return [
    ...
    "- Final standalone frontend/backend extraction and backend release tagging are not complete. The frontend consumer readiness and prepared-artifact install/build proof, extraction manifest, dry-run extraction plan, extracted-workspace readiness model, extracted-install proof harness, SQL ownership inventory, migration bundle manifest, migration index, standalone JWT/JWKS verifier, and bounded JWKS cache tests are checked guardrails only; they do not create or populate the future frontend or backend repositories, execute SQL, create a database, prove RLS/tenant isolation, prove durable idempotency behavior against a database, configure a live auth provider, prove provider operational key rotation, deploy a backend, publish packages, prove registry install, or prove live parity.",
