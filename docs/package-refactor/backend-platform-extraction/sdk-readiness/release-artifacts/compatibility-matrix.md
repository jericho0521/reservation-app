...
| Registry install status | Disposable strict proof serves exact `@reservation-platform/sdk@0.2.0`, `@reservation-platform/contract-types@0.2.0`, `@reservation-platform/react@0.2.0`, and `@reservation-platform/ui@0.2.0` artifacts from a temporary local npm-compatible registry, installs them without overrides into an external temp consumer, and typechecks SDK, contract, React, and visual-preset imports. Private registry and public npm publication are not required for the bundled self-hosted release path. |
