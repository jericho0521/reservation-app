...
export const browserSmokeBaseUrl = "https://reservation-platform-vite-smoke.test";
export const browserSmokeTenantId = "tenant_vite_react";
export const browserSmokeVenueId = "venue_vite_react";
export const browserSmokeAccessToken = "public-demo-token";
export const browserSmokeServiceId = "screening-room";

export interface BrowserFakePlatform {
  fetch: typeof fetch;
  observedContexts: string[];
}

export function createBrowserFakePlatformFetch(): BrowserFakePlatform {
  ...
  const fetchImpl: typeof fetch = async (input, init) => {
    ...
    invariant(url.origin === new URL(browserSmokeBaseUrl).origin, "unexpected smoke backend origin");
    ...
    if (authorization !== `Bearer ${browserSmokeAccessToken}`) {
      return platformError("unauthorized", "Browser demo token is missing or invalid.", 401);
    }
    if (tenantId !== browserSmokeTenantId || venueId !== browserSmokeVenueId) {
      return platformError("bad_request", "Browser smoke tenant or venue header is missing.", 400);
    }
    ...
    if (url.pathname === "/v1/venues" && request.method === "GET") {
      const response: ListVenuesResponse = {
        venues: [{
          venue_id: browserSmokeVenueId,
          tenant_id: browserSmokeTenantId,
          name: "Vite React Smoke Venue",
    ...
    if (url.pathname === "/v1/services" && request.method === "GET") {
      invariant(url.searchParams.get("venue_id") === browserSmokeVenueId, "services query missed venue");
      const response: ListServicesResponse = {
        services: [{
          service_id: browserSmokeServiceId,
          venue_id: browserSmokeVenueId,
          name: "Browser SDK Screening",
          duration_minutes: 90,
          total_quantity: 4,
          resource_kind: "seat",
    ...
    if (url.pathname === "/v1/resources" && request.method === "GET") {
      invariant(url.searchParams.get("service_id") === browserSmokeServiceId, "resources query missed service");
      const response: ListResourcesResponse = {
        resources: [
          { resource_id: "seat-a1", service_id: browserSmokeServiceId, label: "A1", kind: "seat", is_active: true, capacity: 1 },
          { resource_id: "seat-a2", service_id: browserSmokeServiceId, label: "A2", kind: "seat", is_active: true, capacity: 1 },
        ],
      };
      return jsonResponse(response);
    }
    ...
    if (url.pathname === "/v1/availability" && request.method === "GET") {
      invariant(url.searchParams.get("service_id") === browserSmokeServiceId, "availability query missed service");
    ...
    if (url.pathname === "/v1/reservations" && request.method === "POST") {
      ...
      const reservation: ReservationResponse = {
        reservation_id: `res_vite_react_${reservations.size + 1}`,
        tenant_id: browserSmokeTenantId,
        venue_id: browserSmokeVenueId,
    ...
    if (reservationMatch && request.method === "GET") {
      ...
      if (!reservation) {
        return platformError("reservation_not_found", "Reservation was not found.", 404, "req_vite_missing");
      }
      return jsonResponse(reservation);
    }

    return platformError("not_found", `Unhandled browser smoke route ${url.pathname}.`, 404);
  };
...
function platformError(code: PlatformErrorBody["code"], message: string, status: number, requestId = "req_vite_smoke") {
  return jsonResponse({
    error: {
      code,
      message,
