...
export default async function ReservationsPage({ searchParams }: { searchParams: Promise<{ q?: string; status?: string; channel?: string; service?: string }> }) {
  try {
    ...
    return <div className="page-stack"><header className="page-header split-header"><div><span className="eyebrow">Reservations</span><h1>Find every booking</h1><p>Search customers, filter operational state and channel, then inspect or cancel with an audit reason.</p></div><span className="status-pill">{reservations.length} shown</span></header><ReservationFilters services={services.services} values={filters} /><ReservationList reservations={reservations} /></div>;
