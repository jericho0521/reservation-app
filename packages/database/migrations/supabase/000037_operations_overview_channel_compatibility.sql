-- Preserve the venue-scoped operations overview after bookings gained a channel column.
...
as $$
  with scope as (
    ...
    select bookings.*, venue_services.name as service_name,
      coalesce(conversation.channel, bookings.channel, case when bookings.interface_type = 'chat' then 'web_chat' else 'web_booking' end) as resolved_channel
    ...
    'reservations', jsonb_build_object(
      ...
      'timeline', coalesce((
        select jsonb_agg(jsonb_build_object(
          'reservation_id', bounded.id,
          'service_name', bounded.service_name,
          'customer_name', bounded.user_name,
          'start_time', bounded.start_time,
          'end_time', bounded.end_time,
          'quantity', bounded.seats_booked,
          'status', bounded.status,
          'channel', bounded.resolved_channel
        ) order by bounded.start_time, bounded.id)
