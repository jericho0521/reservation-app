...
| `scripts/start-local-supabase.ps1`, `scripts/stop-local-supabase.ps1` | Backend operations helpers | `scripts` | Copy candidates; require generalization | Private backend operations scripts | No | No current frontend deployment assumptions or hard-coded Project Play environment naming. |
